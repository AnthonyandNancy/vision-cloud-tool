...
async function fetchUrlBytes(url: string, signal: AbortSignal, allowExtensionlessImageUrls: boolean): Promise<ResolvedImageBytes> {
  ...
  try {
    ...
    try {
      ...
      if (leaf.length > 0 && leaf !== '/') name = leaf
    } catch {
      name = 'image'
    }
...
export class VisionToolkitRuntime {
  ...
  get model(): { provider: string; model: string } | undefined {
  ...
  private async run<T>(
    ...
    try {
      ...
      if (deadline.signal.aborted) {
        if (deadline.cancelled) throw new VisionToolkitError('cancelled', 'vision_cloud_tool: cancelled')
  ...
  private async readBytes(
    ...
    signal: AbortSignal,
    warnings: string[],
  ): Promise<{ images: ImageInfo[]; result: VisionResult }> {
    ...
    for (const source of sources) {
      ...
      if (!Number.isSafeInteger(pixels) || pixels > this.config.maxImagePixels) {
        throw new VisionToolkitError(
          'capacity',
          `image is ${header.width}x${header.height} (${pixels} pixels), exceeding maxImagePixels ${this.config.maxImagePixels}`,
        )
      }
  ...
  async read(request: VisionCloudRequest, prompt: string | undefined, options: ToolCallOptions): Promise<VisionCloudResult> {
    return this.run(options, async (signal) => {
      ...
      this.ctx.logger.info(
        ...
        Date.now() - started,
        resolved.length,
        this.config.model?.model ?? 'unknown',
      )
      return {
        images: resolved,
        result,
  ...
  async selfTest(options: ToolCallOptions): Promise<VisionCloudResult> {
    return this.run(options, async (signal) => {
      const warnings: string[] = []
