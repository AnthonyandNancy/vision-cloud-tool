  ...
  return (
    <div className="dvt-settings">
      ...
      <section className="dvt-panel dvt-essential">
        <div className="dvt-panel-title"><div><h3>{t('model')}</h3><p>{t('modelHint')}</p></div><span className={`dvt-badge ${snapshot.enabled ? 'ok' : 'error'}`}>{snapshot.enabled ? t('modelName') : t('off')}</span></div>
        <div className="dvt-form-grid">
          <Field label={t('provider')}>
            <select
              ...
              onChange={(event) => { update('provider', event.target.value); update('model', '') }}
            >
              <option value="">{t('off')}</option>
              {providers.map(entry => <option key={entry.provider} value={entry.provider}>{entry.name}</option>)}
            </select>
          </Field>
          <Field label={t('modelName')}>
            <select
              aria-label={t('modelName')}
              disabled={!snapshot.writable || busy || draft.provider === ''}
              value={draft.model}
      ...
      <details className="dvt-advanced">
        ...
        <div className="dvt-advanced-body">
          <section className="dvt-panel"><div className="dvt-form-grid">
            <Field label={t('language')}><select aria-label={t('language')} disabled={busy} value={draft.language} onChange={(event) => { update('language', event.target.value as 'zh' | 'en') }}><option value="zh">中文</option><option value="en">English</option></select></Field>
