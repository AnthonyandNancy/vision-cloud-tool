...
interface LlmCapabilitySurface {
  resolveModelInfo?: (provider: string, model: string, signal?: AbortSignal) => PromiseLike<unknown> | unknown
  listModels?: (provider: string) => PromiseLike<unknown> | unknown
}

function classify(input: unknown): ModelCapability {
  if (!Array.isArray(input)) return 'unknown'
  const modalities = input.filter((value): value is string => typeof value === 'string' && value.trim() !== '')
  if (modalities.length === 0) return 'unknown'
  return modalities.includes('image') ? 'image' : 'text'
}

function exactCatalogCapability(value: unknown, model: string): ModelCapability {
  if (!Array.isArray(value)) return 'unknown'
  for (const entry of value as ModelEntryLike[]) {
...
export async function resolveModelCapability(
  ...
  if (typeof llm !== 'object' || llm === null) return 'unknown'
  const surface = llm as LlmCapabilitySurface
  ...
  if (typeof surface.resolveModelInfo === 'function') {
    try {
      const info = hasSignalArgument
        ? await surface.resolveModelInfo(provider, model, signal) as ModelInfoLike
        : await surface.resolveModelInfo(provider, model) as ModelInfoLike
