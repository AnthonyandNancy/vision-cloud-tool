...
import {
  ...
  type ConversationVisionCapability,
  type VisionImageInputs,
} from './vision-context.ts'

/** Whether the tool is visible for one scope through the registry (ignores presentation collapse). */
export function isVisionToolVisible(ctx: Context, scope: object | undefined): boolean {
  try {
...
function hasNativeVisionTool(assembled: PromptAssembly): boolean {
...
function routeInputs(inputs: VisionImageInputs, capability: ConversationVisionCapability): VisionImageInputs {
  if (capability !== 'image') return inputs
 ...
 * authoritative.
 */
export async function applyVisionPromptEnrichment(
  ctx: Context,
  assembled: PromptAssembly,
