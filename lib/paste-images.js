...
const FORMAT_MEDIA_TYPE_BY_EXTENSION = {
    '.png': 'image/png',
    '.jpg': 'image/jpeg',
    '.jpeg': 'image/jpeg',
    '.gif': 'image/gif',
 ...
 */
export function hashedPastedImageName(raw, mediaType, digest) {
    if (!/^[0-9a-f]{16,}$/iu.test(digest))
        throw new TypeError('hashedPastedImageName requires a SHA-256 hex digest');
    ...
    if (stem.toLowerCase().startsWith(previousPrefix))
        stem = stem.slice(previousPrefix.length);
    if (stem === '' || GENERIC_IMAGE_STEM_RE.test(stem))
        return `${prefix}${extension}`;
    const stemBudget = Math.max(1, MAX_NAME_BYTES - Buffer.byteLength(`${previousPrefix}${extension}`));
    while (Buffer.byteLength(stem) > stemBudget)
        stem = stem.slice(0, -1);
 ...
 */
async function publishStagedImage(stagingPath, finalPath) {
    try {
        const existing = await lstat(finalPath);
        if (!existing.isFile())
            throw new Error(`pasted-image target is not a regular file: ${finalPath}`);
...
async function writeImage(req, directory, rawName, mediaType, expectedBytes, maxBytes) {
...
export class PastedImageBackend {
     ...
     */
    async takeoverVerdict(sessionId, label, pair) {
        ...
        const hasCatalog = llm !== undefined
    ...
    currentModelFromSession(sessionId) {
     ...
     */
    async takeoverForExact(provider, model) {
        const llm = this.ctx.llm;
        try {
            if (typeof llm.resolveModelInfo === 'function') {
                const resolved = await llm.resolveModelInfo(provider, model);
                if (Array.isArray(resolved?.inputModalities) && resolved.inputModalities.includes('image'))
                    return false;
                return true;
            }
        }
        ...
        try {
            ...
            for (const entry of models) {
                if (entry?.id !== model && entry?.name !== model)
                    continue;
                if (Array.isArray(entry?.inputModalities) && entry.inputModalities.includes('image'))
                    return false;
                return true;
            }
        }
     ...
     */
    async handleImageFile(req, res, url) {
        if (req.method !== 'GET') {
        ...
        try {
            ...
            try {
                ...
                const code = error?.code;
                if (code === 'ENOENT')
                    throw new RangeError(`pasted image not found: ${name}`);
                throw error;
            }
