 ...
 */
function assertImageUrlShape(url) {
    let pathname;
    try {
    ...
    if (extension !== '' && !SUPPORTED_IMAGE_EXTENSION_SET.has(extension)) {
        throw new VisionToolkitError('input', `URL does not point to a supported image format ("${extension}"); supported: ${[...SUPPORTED_IMAGE_EXTENSION_SET].join(', ')}: ${url}`);
    }
}
/**
