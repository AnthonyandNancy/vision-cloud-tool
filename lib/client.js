...
class PasteImageController {
    ...
    async settlePaste(sessionId, pick, input, files, text, start, end, target, kind) {
        try {
    ...
    handlePaste(event) {
        ...
        if (this.verdictKey(String(sessionId), pick) === undefined) {
            ...
            if (snapshot.phase === 'plain')
                this.releaseNatively(input, files, text, start, end, target, 'paste');
            return true;
        }
    ...
    handleDrop(event) {
        ...
        if (this.verdictKey(String(sessionId), pick) === undefined) {
            if (snapshot.phase === 'plain')
                this.releaseNatively(input, files, text, start, end, textarea, 'drop');
            return true;
        }
...
function PasteThumb(props) {
    ...
    if (url === '')
        return null;
    return (0, jsx_runtime_1.jsx)("img", { className: "dvt-paste-thumb", src: url, alt: "", "aria-hidden": "true", style: { width: 28, height: 28, objectFit: 'cover', borderRadius: 4, flex: 'none' } });
}
...
function installPasteImages(ctx) {
    ...
    ctx.slots.inject('conversation.chat.node', () => {
        const disposeUser = ctx.slots.register({
            ...
            priority: -1,
            locale: 'conversation',
        }, user_message_view_tsx_1.UserMessageNodeShadow);
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserMessageNodeShadow = exports.BRIDGE_FILE_PREFIX = void 0;
...
function BridgeImageCell(props) {
    ...
    return (0, jsx_runtime_1.jsxs)(jsx_runtime_1.Fragment, { children: [(0, jsx_runtime_1.jsx)("button", { type: "button", className: "dvt-img-frame", "data-variant": props.variant, title: props.labels.open, "aria-label": props.labels.openNamed(props.name), onClick: () => { setOpen(true); }, children: (0, jsx_runtime_1.jsx)("img", { src: props.item.url, alt: props.name, onError: () => { setFailed(true); } }, attempt) }), open && (0, jsx_runtime_1.jsx)(Lightbox, { src: props.item.url, alt: props.name, dialog: props.labels.preview, close: props.labels.closePreview, onClose: () => { setOpen(false); } })] });
}
