 ...
 */
import { normalizeDshFileReference } from "./file-references.js";
export const EMPTY_VISION_IMAGE_INPUTS = {
    attachments: [],
    paths: [],
    urls: [],
};
...
function readAttachmentId(value) {
    if (!isRecord(value))
        return undefined;
...
function collectTextInputs(text, inputs) {
    ...
    for (const match of text.matchAll(DSH_IMAGE_FILE_REFERENCE_PATTERN)) {
        ...
        if (reference.kind !== 'file' || !/\.(?:png|jpe?g|gif|webp)$/iu.test(reference.value))
            continue;
        if (!inputs.paths.includes(reference.value))
            inputs.paths.push(reference.value);
    }
    for (const match of text.matchAll(IMAGE_URL_PATTERN)) {
        const url = (match[0] ?? '').replace(/[.,;:!?]+$/u, '');
        if (url !== '' && !inputs.urls.includes(url))
 ...
 */
function walkSessionValue(value, inputs, seen, depth) {
    if (depth <= 0)
        return;
    if (Array.isArray(value)) {
        for (const item of value)
            walkSessionValue(item, inputs, seen, depth - 1);
        return;
    }
    ...
    for (const key of ['content', 'message', 'data', 'result', 'tool_result', 'events']) {
...
function isUserCarrier(event) {
    if (!isRecord(event))
        return false;
    const type = event.type;
    if (typeof type === 'string' && type.includes('user'))
        return true;
    const data = event.data;
    if (!isRecord(data))
        return false;
    if (data.kind !== undefined && data.kind !== 'user')
        return false;
    return true;
}
/** One message's content, tolerating `data.content` and `data.message.content`. */
function messageContentOf(event) {
    if (!isRecord(event))
        return undefined;
    const data = event.data;
    if (!isRecord(data))
 ...
 * re-read tool output.
 */
export function collectImageInputs(session, depth = 24) {
    if (session === undefined || !Array.isArray(session.events))
        return structuredClone(EMPTY_VISION_IMAGE_INPUTS);
    const inputs = { attachments: [], paths: [], urls: [] };
    const seen = new WeakSet();
    for (const event of session.events) {
        if (!isUserCarrier(event))
            continue;
 ...
 */
export function renderVisionImageContext(inputs, capability) {
    ...
    if (inputs.urls.length > 0)
        lines.push(...inputs.urls.map(url => `- ${url}`));
    ...
    const args = [];
    if (lines.length > 0)
        args.push(`vision_cloud_tool.images:
${lines.join("\n")}`);
...
//# sourceMappingURL=vision-context.js.map