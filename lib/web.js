 ...
 */
import { PASTE_IMAGE_FILE_ROUTE, PASTE_IMAGES_ROUTE } from "./paste-images.js";
...
export class VisionToolkitWebBackend {
                ...
                models = await Promise.all(listed.map(async (model) => {
                    ...
                    try {
                        const resolved = typeof this.ctx.llm.resolveModelInfo === 'function'
                            ...
                            : undefined;
                        reasoningEfforts = (resolved?.reasoning?.efforts ?? [])
    ...
    async assertVisionCapableModel(provider, model) {
    ...
    async save(request) {
        ...
        const current = descriptorOf(this.ctx).value;
        const value = {
            ...request.value,
        ...
        if (value.model !== undefined && value.model.provider !== undefined && value.model.model !== undefined) {
            await this.assertVisionCapableModel(value.model.provider, value.model.model);
        }
