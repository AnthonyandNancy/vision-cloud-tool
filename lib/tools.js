...
export function createVisionCloudTool(runtime, lifecycleSignal) {
    return defineTool({
        ...
        description: 'Read and analyze one or more images with the DSH app\'s configured vision model, returning structured evidence '
            + '(summary, full transcription, layout regions, semantics, visual clues, uncertainty). Use this tool only when the image is NOT directly visible '
            + 'to you in the conversation: you are a text-only model, or the message carries only a workspace path, an http(s) URL ending in '
            + '.png/.jpg/.jpeg/.gif/.webp, or a pasted image attachment (its sha256:... id) — or the user asks to describe, question, OCR, re-analyze, or '
            + 'compare images. If the conversation already contains image content that you can see directly, analyze the image(s) yourself and do NOT call '
